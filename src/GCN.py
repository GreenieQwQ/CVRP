...
class GCN(nn.Module):
    def __init__(self, node_input_dim, edge_input_dim, node_hidden_dim, edge_hidden_dim, gcn_num_layers, k, device):
        ...
        # TODO: what is 4 ??
        self.node_W1 = nn.Linear(4, self.node_hidden_dim)
        self.node_W2 = nn.Linear(4, self.node_hidden_dim // 2)
        self.node_W3 = nn.Linear(1, self.node_hidden_dim // 2)
        self.edge_W4 = nn.Linear(2, self.edge_hidden_dim // 2)
    ...
    def forward(self, node, demand, timewin, dis, timedis):
        ...
        self_edge = torch.arange(0, node_num).unsqueeze(0).repeat(batch_size, 1).to(device)
        order = dis.sort(2)[1]
        ...
        e = self.Relu(torch.cat([self.edge_W4(edge), self.edge_W5(a.unsquezze(3))], dim=3))
